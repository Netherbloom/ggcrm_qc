...
@Repository
public class GoodsDao extends BaseDaoWithQuery<Goods, Integer> {
	...
	public Page<Goods> findPageByName(String keyword,Pageable pageable) {
		...
		MapQuery<Goods> mq = new MapQuery<Goods>(Goods.class);
		return new Page<Goods>(mq.getResultList(query), pageable, intTotal);
	}
	 ...
	 * @return
	 */
	public List<Goods> getByBarcode(String barcode){
		String sql = "select a.id,(select (select b.imgpath from gggoodsimage b where b.goodsid = c.goodsid and length(c.isbn) = 13) as cover from gggoodsisbn c where c.isbn = a.barcode) as cover,a.name,a.writer,a.price,a.cbdate,a.barcode from goods a where 1=1";
		...
		MapQuery<Goods> mq = new MapQuery<Goods>(Goods.class);
		List<Goods> list = mq.getResultList(query);
